                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-center text-sm font-medium">
                                    @if($item->status === 'menunggu')
                                        <div class="flex items-center justify-center space-x-2">
                                            <form action="{{ route('admin.peminjaman.approve', $item->id) }}" method="POST" class="inline" onsubmit="return confirm('Setujui peminjaman ini?');">
                                                @csrf
                                                <button type="submit" class="text-white bg-green-500 hover:bg-green-600 px-3 py-1 rounded shadow-sm text-xs font-semibold">
                                                    Setujui
                                                </button>
                                            </form>
                                            <form action="{{ route('admin.peminjaman.reject', $item->id) }}" method="POST" class="inline" onsubmit="return confirm('Tolak peminjaman ini?');">
                                                @csrf
